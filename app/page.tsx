    ...
    <main className="world">
      ...
      <header className="nav">
        ...
        <nav aria-label="Primary">
          <a href="#watch">Watch</a><a href="#play">Play</a><a href="#read">Read</a><a href="#parents">Parents</a>
        </nav>
      </header>
      <section className="hero" aria-labelledby="title">
        <div className="kotton-orb" role="img" aria-label="Kotton floating in Cotton's cloud world">🐾</div>
      ...
      <section id="studio" className="studio" aria-labelledby="studio-title">
        ...
        <div className="lanes">
          {lanes.map(([title, description, action], index) => <article className={`lane lane-${index + 1}`} id={title.toLowerCase()} key={title}>
            <span className="lane-number">0{index + 1}</span><h3>{title}</h3><p>{description}</p><button type="button">{action} <span aria-hidden="true">↗</span></button>
          </article>)}
        </div>
      </section>
